...
```{r}
census_clean <- census_import %>% 
    select(c("GEOID", "NAME", "hhincome", "medage","pop", "White", "Black", "Native","Asian")) %>% 
...
```
## Question 2:
Based on the average value you get in Question 2 preps, calculate the percentage that the county's median household income is higher than the average. You just need to report the percentage as your answer. 

```{r}
n <- census_clean %>% filter(hhincome>48994.97) %>% count
n/3220
```

# Question 3:
Which state has the highest average median household income? What is the average household income for that state?
...
```{r}
census_clean %>% group_by(State) %>% 
    summarise(ave_income=mean(hhincome,na.rm=T)) %>% arrange(desc(ave_income))
```
...
```{r eval=F}
census_clean %>% mutate(pec_asian=(Asian/pop)) %>%  arrange(desc(pec_asian))
...
General function workflow: group by, filter, select and arrange. 
```{r}
max_pop <- census_clean %>% group_by(State) %>% 
    filter(pop==max(pop)) %>% 
    select("GEOID", "County", "State", "pop") %>% arrange(State)
```
